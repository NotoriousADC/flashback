...
//Used to prevent the dirty bit from being set when writing permission.
BOOL writingPerm = FALSE;
...
STATUS DATA_Write(PGADDR addr, void* buffer, BOOL is_hot) {
  ...
  if (buffer != NULL) {
    ...
    //If we're trying to cache whatever is being written
    if(setup && !isRollingBack && FB_isPermission(addr,(UINT8*) buffer))
    {
      PMT_Search(addr, &block2, &page2);
      if(block2 != 0 && page2 != 0)
      {
        meta_data[page][2] = block2*64+ page2;
        meta_data[page][3] = currentHead;
        writingPerm = TRUE;
      }
    }
